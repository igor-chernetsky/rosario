...
ColorReductionResult reduceColorsWithMapping(
    List<Color> colors, int targetCount) {
  ...
  if (colors.length <= targetCount) {
    ...
    Map<Color, Color> mapping = {for (var color in colors) color: color};
...
ClusteringResult _kMeansClusteringWithMapping(List<LabColor> colors, int k,
    {int maxIterations = 100}) {
  if (colors.isEmpty || k <= 0) return ClusteringResult([], {});
  if (colors.length <= k) {
    Map<LabColor, LabColor> mapping = {for (var color in colors) color: color};
...
LabColor rgbToLab(Color color) {
  ...
  x = (x > 0.008856 ? pow(x, 1 / 3) : (7.787 * x) + (16 / 116)).toDouble();
  y = (y > 0.008856 ? pow(y, 1 / 3) : (7.787 * y) + (16 / 116)).toDouble();
  z = (z > 0.008856 ? pow(z, 1 / 3) : (7.787 * z) + (16 / 116)).toDouble();
...
Color labToRgb(LabColor lab) {
  ...
  double x3 = pow(x, 3).toDouble();
  double y3 = pow(y, 3).toDouble();
  double z3 = pow(z, 3).toDouble();
